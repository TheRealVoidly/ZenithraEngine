...
struct ObjectData* zenithra_load_obj(struct InEngineData *engine_data_str, bool engine_obj, const char* object_file_name, const char* texture_file_name){
	...
	int vi = face_buffer_data[0] - 1;
	obj->x_max = temp_vertex_buffer_data[vi*3];
	obj->x_average = temp_vertex_buffer_data[vi*3];
	obj->y_max = temp_vertex_buffer_data[vi*3+1];
	obj->y_average = temp_vertex_buffer_data[vi*3+1];
	obj->z_max = temp_vertex_buffer_data[vi*3+2];
	obj->z_average = temp_vertex_buffer_data[vi*3+2];
	int n = 1;
	for(int i = 0; i < (obj->obj_size * 9) - 1; i+=3){
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * obj->obj_size * 9, &obj->vertex_buffer_data[0], GL_STATIC_DRAW);

	glGenBuffers(1, &obj->obj_uv_buffer);
	glBindBuffer(GL_ARRAY_BUFFER, obj->obj_uv_buffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * obj->obj_size * 6, &uvs_buffer_data[0], GL_STATIC_DRAW);

	glGenBuffers(1, &obj->obj_normal_buffer);
	glBindBuffer(GL_ARRAY_BUFFER, obj->obj_normal_buffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * obj->obj_size * 9, &normals_buffer_data[0], GL_STATIC_DRAW);
...
void zenithra_render_object(struct InEngineData *engine_data_str, struct ObjectData **obj, int obj_num){
	...
	glDrawArrays(GL_TRIANGLES, 0, obj[obj_num]->obj_size * 3);
